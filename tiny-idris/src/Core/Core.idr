...
data Error : Type where
  CantConvert :
    {vars : List Name} -> Env Term vars -> Term vars -> Term vars -> Error
...
Show Error where
  ...
  show (CaseCompile n (MatchErased (_ ** (env, tm)))) =
    "Attempt to match on erased argument " ++ show tm ++ " in " ++ show n
  show (CaseCompile n (NotFullyApplied c)) =
    "Constructor " ++ show c ++ " is not fully applied"
  show (FileErr fname err) =
    "File error (" ++ fname ++ "): " ++ show err

-- | NOTE Core
-- a wrapper around IO that is specialised for efficiency.
export
record Core t where
  constructor MkCore
...
-- | NOTE [| f a1 …an |] is translated into `pure f <*> a1 <*> … <*> an`
export
...
data Ref : (l : label) -> Type -> Type where
  [search l] -- NOTE start searching when l is figured out
  MkRef : IORef a -> Ref x a

export
newRef : (x : label) -> t -> Core (Ref x t)
newRef x val = do
  ref <- coreLift (newIORef val)
  pure (MkRef ref)

-- | NOTE there could be many (Ref _ _) in scope but as long as
-- | (x : label) is unique, the correct Ref can be found automatically
export %inline
...
condC [] def = def
condC ((x, y) :: xs) def = if !x then y else condC xs def
