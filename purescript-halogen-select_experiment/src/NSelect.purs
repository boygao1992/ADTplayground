...
scrollIntoViewIfNeeded index = do
  void $ runMaybeT do
    ...
    -- let selector = QuerySelector $ "[data-nselect-item='" <> show index <> "']"
    -- itemEl
    --   <- MaybeT <<< H.liftEffect
    --   $ querySelector selector
    --   $ HTMLElement.toParentNode menu
    -- item <- MaybeT <<< pure $ HTMLElement.fromElement itemEl
    ...
    H.lift $ H.liftEffect do
      ...
      --   then do
      --     Element.setScrollTop (itemOffsetTop + itemOffsetHeight - menuHeight) menuEl
      --   else
      --     if itemOffsetTop < scrollTop
      --     then do
      --       Element.setScrollTop itemOffsetTop menuEl
      --     else pure unit
