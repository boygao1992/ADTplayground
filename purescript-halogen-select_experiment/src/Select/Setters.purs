...
import Select (Action(..), Target(..), Visibility(..))
...
setContainerProps
  :: forall props act
  . Array (HP.IProp (ContainerProps props) (Action act))
  -> Array (HP.IProp (ContainerProps props) (Action act))
setContainerProps = append
  -- [ HE.onMouseDown $ Just <<< PreventClick ]
  [ HE.onMouseDown $ Just <<< OnMouseDownContainer
  , HE.onMouseUp $ Just <<< OnMouseUpContainer
  ]
