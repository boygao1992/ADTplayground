...
dropdownContainer
  :: ∀ p act item
  . Array (HH.HTML p (Select.Action act))
  -> (item -> HH.PlainHTML)
  -> (item -> Boolean)
  -> Array item
  -> Maybe Int
  -> HH.HTML p (Select.Action act)
dropdownContainer addlHTML renderItem selected items hix =
  ...
  where
    renderItems :: Array (HH.HTML p (Select.Action act))
    renderItems =
      [ HH.ul
    ...
    renderItem' :: Int -> item -> HH.HTML p (Select.Action act)
    renderItem' idx item =
      dropdownItem HH.li
