...
import Formless.Types.Form (FormField(..), FormFieldRow, InputField(..), InputFunction, OutputField(..))
import Formless.Data.FormFieldResult (FormFieldResult(..), fromEither, toMaybe)
import Formless.Validation (FormFields, FormInputFields, FormInputFunction, FormInputFunctions, FormOutputFields, FormValidationAction, FormValidators, Validation, runValidation)
...
unsafeModifyInputVariant
  ...
  -> FormFields form
  -> FormFields form
unsafeModifyInputVariant modifyResult var rec
  = wrap $ unsafeSet label val (unwrap rec)
  where
    ...
    val =
      let
        modifyInput :: i -> i
        modifyInput = unwrap (snd rep)
      in case unsafeGet (fst rep) (unwrap rec) of
        FormField x -> FormField $ x
          { input = modifyInput x.input, result = modifyResult x.result }
